...
import {DeployLiquidateQualifiedUser} from "script/DeployLiquidateQualifiedUser.s.sol";
...
contract LiquidateQualifiedUserTest is Test {
    ...
    function testUserQualifiedForLiquidation() public {
        LiquidateQualifiedUser.User memory user =
    ...
    function testLiquidateRevertsWithNoDebt() public {
        LiquidateQualifiedUser.User memory user = setupUser(USER, wethAddress, address(usdt_token), SUPPLY_WETH, 0);
        // creat aave user account
        vm.startPrank(USER);
        vm.expectRevert(LiquidateQualifiedUser.NoUserAccountQualifiedForLiquidation.selector);
    ...
    function setupUser(
    ...
    ) public returns (LiquidateQualifiedUser.User memory) {
        ...
        // TODO - acccount for liquidationFactor ( 0.5 or 1)
        return LiquidateQualifiedUser.User({
            id: USER,
            debtToken: address(borrowToken),
            collateralToken: address(supplyToken),
            debtToCover: borrowAmount / 2
        });
    }
}
